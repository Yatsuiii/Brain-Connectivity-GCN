...
from graph_temporal_net import graph_temporal_netCell
...
flags.DEFINE_string('model_name', 'graph_temporal_net', 'graph_temporal_net')
...
def GraphTemporalNet(_X, _weights, _biases):
    ###
    cell_1 = graph_temporal_netCell(gru_units, adj, num_nodes=num_nodes)
...
if model_name == 'graph_temporal_net':
    pred,ttts,ttto = GraphTemporalNet(inputs, weights, biases)
...
for epoch in range(training_epoch):
    ...
    if (epoch % 500 == 0):        
        saver.save(sess, path+'/model_100/GraphTemporalNet_pre_%r'%epoch, global_step = epoch)
